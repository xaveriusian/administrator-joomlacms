...
class VideoTable extends Table {	
	...
	var $upload_dir = 'media/com_allvideoshare/videos';
	...
	public function bind( $array, $ignore = '' ) {
		...
		if ( ! Factory::getUser()->authorise( 'core.admin', 'com_allvideoshare.video.' . $array['id'] ) ) {
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_allvideoshare/access.xml',
				"/access/section[@name='video']/"
			);
			$default_actions = Access::getAssetRules( 'com_allvideoshare.video.' . $array['id'] )->getData();
			$array_jaccess   = array();
	...
	public function check()	{
		...
		if ( $files['hd']['size'] > 0 ) {
			...
			if ( $fileError > 0 && $fileError != 4 ) {
				...
				$this->hd = Uri::root() . $this->upload_dir . '/' . $date . '/' . $filename;
			}
		} else {
			if ( isset( $array['hd'] ) ) {
				$this->hd = $array['hd'];
    ...
    public function delete( $pk = null ) {
		...
		if ( $result )	{
			...
			$isUploaded = strpos( $this->hd, $this->upload_dir );

			if ( $isUploaded !== false ) {
				// Remove protocols
				$parts = explode( $this->upload_dir, $this->hd );
				$file = JPATH_ROOT . '/' . $this->upload_dir . $parts[1];
